...
import { Home, BookOpen, AlertCircle, ClipboardList, ShoppingBag, LogIn, ArrowLeft } from "lucide-react";
  ...
  return (
    <div style={{minHeight:"100dvh",background:"rgb(var(--hi5-bg))"}}>
      ...
      <header style={{
        ...
        <Link to="/self-service" style={{display:"flex",alignItems:"center",gap:8,textDecoration:"none",flexShrink:0}}>
          <img src={logo} alt="Hi5Tech" style={{height:28,width:"auto"}}/>
          <div>
            <div style={{fontSize:13,fontWeight:800,letterSpacing:"-0.02em",lineHeight:1}}>Self Service</div>
        ...
        <nav style={{display:"flex",gap:4,marginLeft:20,flex:1,overflow:"hidden"}}>
          {NAV.map(n=>{
            const active = n.href==="/self-service" ? pathname==="/self-service" : pathname.startsWith(n.href);
            return (
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",marginLeft:"auto",flexShrink:0}}>
          <button type="button" onClick={()=>navigate("/login")}
            ...
            style={{height:34,padding:"0 12px",borderRadius:9,fontSize:12,fontWeight:600,display:"flex",alignItems:"center",gap:5}}>
            <LogIn size={13}/>Staff Login
          </button>
          <button type="button" onClick={()=>navigate(-1)}
            className="hi5-btn-ghost no-min-touch"
            style={{height:34,width:34,padding:0,borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <ArrowLeft size={15}/>
          </button>
        </div>
      </header>
