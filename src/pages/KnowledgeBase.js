...
export default function KnowledgeBase() {
  ...
  const [sort, setSort] = useState({ field: "views", dir: "desc" });

  const stats = useMemo(
    () => ({
      total: KB_ARTICLES.length,
      published: KB_ARTICLES.filter((a) => a.status === "Published").length,
      draft: KB_ARTICLES.filter((a) => a.status === "Draft").length,
      totalViews: KB_ARTICLES.reduce((s, a) => s + a.views, 0),
      avgHelpful: Math.round(
        KB_ARTICLES.reduce((s, a) => s + a.helpful, 0) / KB_ARTICLES.length
      ),
    }),
    []
  );
    ...
    rows.sort((a, b) => {
      const v = (x) =>
        sort.field === "lastUpdated"
          ? new Date(x.lastUpdated)
          : sort.field === "views" || sort.field === "helpful"
          ? x[sort.field]
          : x[sort.field] || "";
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Self-service articles and IT documentation"
        stat={`${stats.published} published`}
      />
      ...
      <Card style={{ marginBottom: 16, padding: "14px 16px" }}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          ...
          />
          <FilterPills
            options={["All", "Published", "Draft", "Under Review"]}
            value={status}
            onChange={setStatus}
          />
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8}>
                    <EmptyState icon="📚" title="No articles found" body="Try adjusting filters." />
                  </td>
                </tr>
              )}
              ...
              {filtered.map((art) => (
                <TableRow key={art.id}>
                      ...
                      {art.tags.map((t) => (
                        <span
                          ...
                          style={{
                            ...
                            padding: "1px 6px",
                            borderRadius: 4,
                            background: "rgb(var(--hi5-accent)/0.08)",
                  ...
                  <TD>
                    <StatusBadge status={art.status} config={STATUS_CFG} />
                  </TD>
                  <TD>
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <Avatar name={art.author} size={24} />
                      <span style={{ fontSize: 12 }}>{art.author}</span>
                    </div>
                  </TD>
                  <TD>
                  ...
                  <TD style={{ fontSize: 12, opacity: 0.4, whiteSpace: "nowrap" }}>
                    {fmt(art.lastUpdated)}
                  </TD>
                </TableRow>
              ))}
        ...
        >
          Showing {filtered.length} of {KB_ARTICLES.length} articles
        </div>
      </Card>
    </div>
  );
